...
class Hmm(val pi: Seq[Double], val T: Array[Array[Double]],
  ...
  /* forward step with log scaling that keeps running sum of log scaling */
  private def logFwdStepWithScale(state: (Seq[Double], Double),
				  obs:   Int): (Seq[Double], Double) = {
  ...
  /* alernate implementation of foward algorithm based on fold */
  ...
  /* Similar variation for the backward algorithm; calculate the scaled
   * probability of seeing the observation sequence having started in a
   * particular state */
  def backward(obsSeq: Seq[Int], startState: Int): Double = {
    val (trellis, scaling) = processObsSeqScaled(Array.fill(numStates)(1.0),
						 obsSeq.tail)
    T(startState)
      .map{ transProb: Double => log(transProb) }
      .zip(trellis)
      .map{ case (logTransProb, stateProb) => logTransProb + stateProb }
      .sum + log(A(startState)(obsSeq.head)) + scaling
  }
