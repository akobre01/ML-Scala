/* This file holds the imperative (non-functional) implementation
 * of my Hmm. It's probably faster but it's also many times longer (code)
 */
 ...
 */
class HmmImp(val pi: Array[Double], val T: Array[Array[Double]],
  ...
  def genObsSeq(steps:Int): List[Int] = {
    ...
    def tailRecGenObs(steps:  Int, currState: Int = inverseSample(pi),
		      obsSeq: List[Int] = List()): List[Int] = steps match {
      case 0     => obsSeq.reverse
      case steps => tailRecGenObs(steps-1, inverseSample(T(currState)),
